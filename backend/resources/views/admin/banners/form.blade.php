    ...
    <form method="POST" enctype="multipart/form-data" action="{{ $banner->exists ? route('admin.banners.update', $banner) : route('admin.banners.store') }}" class="grid gap-6">
        ...
        <div class="rounded-[1.75rem] border border-blue-100 bg-white/90 p-6 shadow-soft">
            <div class="grid gap-5 md:grid-cols-2">
                ...
                <div>
                    <label class="mb-2 block text-sm font-semibold">Gambar Banner</label>
                    <input type="file" name="image" accept=".jpg,.jpeg,.png,.webp" class="w-full rounded-2xl border border-blue-100 px-4 py-3">
                    <p class="mt-2 text-xs leading-6 text-slate-500">Format: jpg, jpeg, png, webp. Maksimal 5MB.</p>
                </div>
                <div>
                    <label class="mb-2 block text-sm font-semibold">Preview Banner</label>
                    <div class="overflow-hidden rounded-[1.25rem] border border-blue-100 bg-surface-soft">
                        <img
                            src="{{ \App\Support\MediaUrl::url($banner->image_path, 'images/default.jpg') }}"
                            alt="Preview banner"
